...
logx50_temperature(t::Real) = 50 / log(t)

function Test33_simulated_annealing(tuning_run::MPRun,
                                   reference::RemoteChannel;
                                   temperature::Function = logx50_temperature)
    name               = "Test33_Simulated Annealing $(myid())"
    iteration          = 1
    cost_calls         = tuning_run.cost_evaluations
    ...
    while !stop
        ...
        result                     = Tests_probabilistic_improvement(tuning_run, threshold = p, distance = 0.3, RandPercentage = 0.5, NbVaribleMutate = 2)
...
function Test34_simulated_annealing(tuning_run::MPRun,
                                   reference::RemoteChannel;
                                   temperature::Function = logx50_temperature)
    name               = "Test34_Simulated Annealing $(myid())"
    iteration          = 1
    cost_calls         = tuning_run.cost_evaluations
    stopping_criterion = @task tuning_run.stopping_criterion(tuning_run.duration)
    stop               = consume(stopping_criterion)
    ...
    while !stop
        iteration                 += 1
        p                          = temperature(iteration)
        result                     = Tests_probabilistic_improvement(tuning_run, threshold = p, distance = 0.3, RandPercentage = 0.0, NbVaribleMutate = 2)
        cost_calls                += result.cost_calls
        result.cost_calls          = cost_calls
        result.start               = tuning_run.starting_point
        result.technique           = (result.technique != "MP Probabilistic Improvement")? result.technique : name
...
function Test35_simulated_annealing(tuning_run::MPRun,
                                   reference::RemoteChannel;
                                   temperature::Function = logx50_temperature)
    name               = "Test35_Simulated Annealing $(myid())"
    iteration          = 1
    cost_calls         = tuning_run.cost_evaluations
    stopping_criterion = @task tuning_run.stopping_criterion(tuning_run.duration)
    stop               = consume(stopping_criterion)
    ...
    while !stop
        iteration                 += 1
        p                          = temperature(iteration)
        result                     = Tests_probabilistic_improvement(tuning_run, threshold = p, distance = 0.3, RandPercentage = 1.0, NbVaribleMutate = 2)
        cost_calls                += result.cost_calls
        result.cost_calls          = cost_calls
        result.start               = tuning_run.starting_point
        result.technique           = (result.technique != "MP Probabilistic Improvement")? result.technique : name
...
logx100_temperature(t::Real) = 100 / log(t)

function Test36_simulated_annealing(tuning_run::MPRun,
                                   reference::RemoteChannel;
                                   temperature::Function = logx100_temperature)
    name               = "Test36_Simulated Annealing $(myid())"
    iteration          = 1
    cost_calls         = tuning_run.cost_evaluations
    stopping_criterion = @task tuning_run.stopping_criterion(tuning_run.duration)
    stop               = consume(stopping_criterion)
...
function Test37_simulated_annealing(tuning_run::MPRun,
                                   reference::RemoteChannel;
                                   temperature::Function = logx100_temperature)
    name               = "Test37_Simulated Annealing $(myid())"
    iteration          = 1
    cost_calls         = tuning_run.cost_evaluations
    stopping_criterion = @task tuning_run.stopping_criterion(tuning_run.duration)
    stop               = consume(stopping_criterion)
...
function Test38_simulated_annealing(tuning_run::MPRun,
                                   reference::RemoteChannel;
                                   temperature::Function = logx100_temperature)
    name               = "Test38_Simulated Annealing $(myid())"
    iteration          = 1
    cost_calls         = tuning_run.cost_evaluations
    stopping_criterion = @task tuning_run.stopping_criterion(tuning_run.duration)
    stop               = consume(stopping_criterion)
...
function Test39_simulated_annealing(tuning_run::MPRun,
                                   reference::RemoteChannel;
                                   temperature::Function = logx50_temperature)
    name               = "Test39_Simulated Annealing $(myid())"
    iteration          = 1
    cost_calls         = tuning_run.cost_evaluations
    stopping_criterion = @task tuning_run.stopping_criterion(tuning_run.duration)
    stop               = consume(stopping_criterion)
    ...
    while !stop
        iteration                 += 1
        p                          = temperature(iteration)
        result                     = Tests_probabilistic_improvement(tuning_run, threshold = p, distance = 0.5, RandPercentage = 0.5, NbVaribleMutate = 2)
        cost_calls                += result.cost_calls
        result.cost_calls          = cost_calls
        result.start               = tuning_run.starting_point
        result.technique           = (result.technique != "MP Probabilistic Improvement")? result.technique : name
...
function Test40_simulated_annealing(tuning_run::MPRun,
                                   reference::RemoteChannel;
                                   temperature::Function = logx100_temperature)
    name               = "Test40_Simulated Annealing $(myid())"
    iteration          = 1
    cost_calls         = tuning_run.cost_evaluations
    stopping_criterion = @task tuning_run.stopping_criterion(tuning_run.duration)
    stop               = consume(stopping_criterion)
