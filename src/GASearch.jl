...
tStart = now()
model = runga(GAforManSim; initial_pop_size = 10)
tEnd = now()
...
include(joinpath( dirname( Base.source_path() ), myid()==1? "": "src", "Types", "Parameters.jl"))
...
for var in 1:length(ParametersList)
    if length(ParametersList[var].TimeDivision) > 2
        for div in 1:(length(ParametersList[var].TimeDivision)-1)
            println(ParametersList[var].Type, " ", var, " ", div," : ", ent.mpSim.recruitmentSchemes[ParametersList[var].Index].RecFlowArray[ParametersList[var].TimeDivision[div] + 1])
        end
    end
end
